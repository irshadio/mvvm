...
/// `remote_client` upgrade adds a case, this fails to compile until mapped.
Failure mapRemoteFailure(rc.Failure failure) {
  final statusCode = failure.response?.statusCode ?? 0;
  final message = failure.errorMessage;

  return switch (failure) {
    rc.NoInternet() => const Failure.noConnection(),
    rc.ConnectionError() ||
    rc.BadCertificate() => Failure.network(message: message),
    ...
    rc.ServiceUnavailable() => Failure.server(
